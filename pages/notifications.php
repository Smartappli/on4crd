...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'mark_all_read') {
        ...
        set_flash('success', (string) $t['all_marked_read']);
    } elseif ($action === 'mark_read') {
        $notificationId = (int) ($_POST['notification_id'] ?? 0);
        if ($notificationId > 0) {
            member_notification_mark_read($memberId, $notificationId);
            set_flash('success', (string) $t['marked_read']);
        }
    }
...
<section class="card notifications-hero member-module-hero">
    <div class="row-between">
        ...
        <span class="badge muted"><?= $unreadCount ?> <?= e((string) $t['unread']) ?></span>
    </div>
</section>
...
<section class="card notifications-list-card">
    <form method="get" class="inline-form" style="margin-bottom:.8rem;flex-wrap:wrap;">
        ...
        <select name="filter">
            <option value="all" <?= $filter === 'all' ? 'selected' : '' ?>><?= e((string) $t['filter_all']) ?></option>
            <option value="unread" <?= $filter === 'unread' ? 'selected' : '' ?>><?= e((string) $t['filter_unread']) ?></option>
        </select>
    ...
    <form method="post" class="inline-form" style="margin-bottom:1rem;">
        ...
        <button class="button secondary" type="submit"><?= e((string) $t['mark_all_read']) ?></button>
    </form>
            ...
            <?php foreach ($notifications as $item): ?>
                ...
                <li class="card" style="margin:0;<?= $isRead ? '' : 'border-color:var(--accent-500);' ?>">
                    <div class="row-between" style="gap:.8rem;">
                        ...
                        <span class="badge muted"><?= e($isRead ? (string) $t['read'] : (string) $t['unread']) ?></span>
                    </div>
                    ...
                    <p class="actions">
                        <?php if (trim((string) ($item['url'] ?? '')) !== ''): ?><a class="button secondary" href="<?= e((string) $item['url']) ?>"><?= e((string) $t['open']) ?></a><?php endif; ?>
                        <?php if (!$isRead): ?>
                            <form method="post" class="inline-form">
                                <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
                                <input type="hidden" name="action" value="mark_read">
                                <input type="hidden" name="notification_id" value="<?= (int) ($item['id'] ?? 0) ?>">
                                <button class="button secondary" type="submit"><?= e((string) $t['mark_read']) ?></button>
                            </form>
                        <?php endif; ?>
                    </p>
                </li>
    ...
    <?php if ($totalPages > 1): ?>
        <nav class="admin-library-pagination" aria-label="<?= e((string) $t['title']) ?>">
            <?php if ($page > 1): ?><a class="button secondary" href="<?= e(route_url_clean('notifications', ['filter' => $filter, 'p' => $page - 1])) ?>">&larr; <?= e((string) $t['prev']) ?></a><?php endif; ?>
            <span class="badge muted"><?= e((string) $t['page']) ?> <?= $page ?> / <?= $totalPages ?></span>
            <?php if ($page < $totalPages): ?><a class="button secondary" href="<?= e(route_url_clean('notifications', ['filter' => $filter, 'p' => $page + 1])) ?>"><?= e((string) $t['next']) ?> &rarr;</a><?php endif; ?>
        </nav>
    <?php endif; ?>
</section>
</div>

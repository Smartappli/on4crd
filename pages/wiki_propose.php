...
set_page_meta([
    ...
    'description' => $tr('propose_meta_desc', 'Cr√©er une nouvelle page wiki depuis l‚Äôespace membre.'),
    'robots' => 'noindex,nofollow',
]);

if (!ensure_wiki_tables()) {
    echo render_layout('<div class="card"><h1>' . e($tr('propose_title', 'Proposer une page wiki')) . '</h1><p>' . e($tr('meta_desc', 'Cr√©er ou modifier une page wiki.')) . '</p></div>', $tr('propose_title', 'Proposer une page wiki'));
    return;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (mb_strlen($title) > 190 || mb_strlen($slugInput) > 190 || mb_strlen($content) > 50000) {
            throw new RuntimeException($tr('error_field_too_long', 'Un des champs d√©passe la longueur autoris√©e.'));
        }
        ...
        set_flash('success', $tr('propose_success', 'Page wiki proposÈe. Elle sera publiÈe aprËs validation.'));
    ...
    <section class="wiki-edit-hero">
        <div>
            ...
            <p class="help"><?= e($autoPublish
                ...
                : $tr('propose_help', 'RÈdigez une nouvelle page avec du HTML simple. Elle sera relue avant publication.')) ?></p>
        </div>

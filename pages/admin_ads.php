...
$pendingAds = cache_remember('admin_ads_pending_v1', 30, static function (): array {
...
$activeAds = cache_remember('admin_ads_recent_v1', 30, static function (): array {
    return db()->query('SELECT a.*, ap.name AS placement_name, ap.code AS placement_code, m.callsign AS owner_callsign
        FROM ads a
        INNER JOIN ad_placements ap ON ap.id = a.placement_id
        LEFT JOIN members m ON m.id = a.owner_member_id
...
$totals = cache_remember('admin_ads_totals_v1', 30, static function (): array {
    return db()->query('SELECT COUNT(*) AS ads_count FROM ads')->fetch() ?: ['ads_count' => 0];
});
$totalImpressions = (int) cache_remember('admin_ads_impressions_total_v1', 30, static function (): int {
    return (int) (db()->query('SELECT COUNT(*) FROM ad_events WHERE event_type = "impression"')->fetchColumn() ?: 0);
});
$totalClicks = (int) cache_remember('admin_ads_clicks_total_v1', 30, static function (): int {

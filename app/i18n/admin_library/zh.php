<?php
declare(strict_types=1);

return array (
  'title' => '资料库管理',
  'intro' => '以 PDF、DOCX、文本、Markdown 或 HTML 格式发布成员文档，提取其内容并按类别组织它们。',
  'category_ph' => '分类',
  'categories' => '分类',
  'existing_categories' => '现有分类',
  'add_category' => '添加分类',
  'delete' => '删除',
  'save' => '保存',
  'title_ph' => '标题',
  'desc_ph' => '摘要或关键词',
  'upload' => '上传文档',
  'open' => '打开文档',
  'preview' => '预览',
  'prev' => '上一页',
  'next' => '下一页',
  'page' => '页',
  'err_required' => '需要标题和文件。',
  'err_invalid' => '文件必须是有效的 PDF、DOCX、TXT、Markdown 或 HTML 文档。',
  'err_size' => '该文档超出了 120 MB 的允许限制。',
  'err_upload' => '文档上传失败。',
  'ok_added' => '文档已添加至成员库。',
  'notification_import_completed_title' => '资料库导入完成',
  'storage_unavailable' => '图书馆暂时无法使用。',
  'meta_desc' => 'ON4CRD 成员库的管理。',
  'documents' => '文档',
  'file' => '文件',
  'file_help' => '接受的格式：PDF、DOCX、TXT、Markdown 和 HTML。最大大小：120 MB。',
  'category_code' => '代码',
  'category_label' => '标签',
  'ok_category' => '类别已保存。',
  'ok_category_deleted' => '类别已删除。其文件已移交给将军。',
  'ok_deleted' => '文档已删除。',
  'empty' => '图书馆中还没有文档。',
  'confirm_delete' => '永久删除此项目？',
  'delete_document' => '删除文档',
  'tags_ph' => '标签',
  'tags_help' => '示例：法律、安全、培训',
  'tags' => '标签',
  'all_categories' => '所有类别',
  'search_ph' => '搜索',
  'tag_search_ph' => '搜索标签',
  'filter' => '筛选',
  'reset' => '重置',
  'bulk_delete' => '删除选择',
  'select' => '选择',
  'tag_cleanup_title' => '标签清理',
  'tag_cleanup_help' => '在审核或导出前规范化重复标签变体。',
  'tag_from_ph' => '来源标签',
  'tag_to_ph' => '目标规范标签',
  'tag_duplicates_empty' => '未检测到重复标签变体。',
  'ok_tags_merged' => '已在 %d 个文档中合并标签。',
  'ingestion_template' => '导入模板',
  'ingestion_template_none' => '无',
  'ingestion_template_training' => '培训',
  'ingestion_template_safety' => '安全',
  'ingestion_template_technical' => '技术',
  'ingestion_template_legal' => '法律',
  'subcategory_ph' => '子主题',
  'subcategories' => '子主题',
  'add_subcategory' => '添加子主题',
  'subcategory_code' => '子主题代码',
  'subcategory_label' => '子主题标签',
  'ok_subcategory' => '子主题已保存。',
  'ok_subcategory_deleted' => '子主题已删除。文档仍保留在父主题中。',
  'all_subcategories' => '所有子主题',
  'no_subcategory' => '无子主题',
  'err_category_has_subcategories' => '请先删除此主题中的所有子主题。',
  'err_subcategory_has_documents' => '此子主题仍包含文档。删除前请先移动或删除这些文档。',
  'pending_proposals_title' => '内容待审核',
  'proposal_status_pending' => '待审核',
  'proposal_status_reviewed' => '已审核',
  'proposal_status_accepted' => '已接受',
  'proposal_status_rejected' => '被拒绝',
  'proposal_type_category' => '主题',
  'proposal_type_tag' => '关键词',
  'proposal_default_title' => '提议',
  'proposal_author' => '提议者',
  'proposal_contact' => '联系方式',
  'proposal_status_label' => '状态',
  'proposal_moderation_note' => '审核说明',
  'proposal_save_status' => '保存状态',
  'proposal_status_saved' => '提案已更新。',
  'proposal_type_content' => '文档',
  'proposal_type_subcategory' => '子主题',
  'pending_proposals_empty' => '没有文档提案正在等待审核。',
  'proposal_open_source' => '打开来源',
  'subsubcategory_ph' => '子子主题',
  'subsubcategories' => '子子主题',
  'add_subsubcategory' => '添加子子主题',
  'subsubcategory_code' => '子子主题代码',
  'subsubcategory_label' => '子子主题标签',
  'ok_subsubcategory' => '子子主题已保存。',
  'ok_subsubcategory_deleted' => '子子主题已删除。文档仍保留在父主题中。',
  'all_subsubcategories' => '所有子子主题',
  'no_subsubcategory' => '无子子主题',
  'err_subcategory_has_subsubcategories' => '请先删除此主题中的所有子子主题。',
  'err_subsubcategory_has_documents' => '此子子主题仍包含文档。删除前请先移动或删除这些文档。',
  'proposal_type_subsubcategory' => '子子主题',

  'confirm_reject_proposal' => '确认拒绝该库提案？这一决定改变了其编辑工作流程。',
);